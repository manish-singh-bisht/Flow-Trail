import { StepSchema, StepStatusSchema, type StepType, type ObservationType } from './types.js';
...
export class Step {
  ...
  public readonly parentStep: StepType['parentStep'];
  ...
  constructor(  
    ...
    this.parentStep = validated.parentStep;
  ...
  capture({name, data, queryable}: {name: ObservationType['name'], data: ObservationType['data'], queryable?: ObservationType['queryable']}): Observation {
    const currentVersion = this._observationVersionMap.get(name) ?? 0;
    const newVersion = currentVersion + 1;
    this._observationVersionMap.set(name, newVersion);

    const observation = new Observation({name, data, queryable, step: this.name, version: newVersion});
